...
[![AI Agent](https://img.shields.io/badge/Agent-Claude_|_Codex_|_OpenClaw-8B5CF6.svg)](.claude/skills/mbeditor/SKILL.md)
...
MBEditor 提供标准化 Agent Skill，安装后 Agent 就能直接操控编辑器。

#### 方式一：npx skills 一键安装（推荐 — 支持 10+ AI Agent 平台）

```bash
# 从 GitHub 直接安装（任意目录可用）
npx skills add AAAAAnson/mbeditor
...
```bash
claude "写一篇关于 Docker 的推文，杂志风排版，发到草稿箱"
```

#### 方式二：手动安装（按 Agent 类型选择）

<details>
<summary><strong>Claude Code</strong></summary>

**项目级安装** — 在 MBEditor 项目目录下直接使用，Agent 会自动发现：
...
cp -r .claude/skills/mbeditor/* ~/.claude/skills/mbeditor/

# Windows
xcopy /E /I .claude\skills\mbeditor %USERPROFILE%\.claude\skills\mbeditor
```
...
cp -r .claude/skills/mbeditor/* ~/.codex/agents/mbeditor/
...
```bash
openclaw skill add .claude/skills/mbeditor/SKILL.md
...
├── .claude/skills/        # AI Agent Skill 定义（标准格式）
│   └── mbeditor/
│       ├── SKILL.md       # 主 Skill 文件（设计原则 + 写作规范）
│       └── api-reference.md  # 完整 API 参考文档
├── skill/                 # 旧路径 → .claude/skills/mbeditor/ 的符号链接（向后兼容）
├── .skills.json           # npx skills 包管理配置
