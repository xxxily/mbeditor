...
[![AI Agent](https://img.shields.io/badge/Agent-Claude_|_Codex_|_OpenClaw-8B5CF6.svg)](skills/mbeditor/SKILL.md)
...
# Windows
xcopy /E /I skills\mbeditor %USERPROFILE%\.claude\skills\mbeditor
```
...
```bash
openclaw skill add skills/mbeditor/SKILL.md
...
├── skills/                # AI Agent Skills 统一管理（标准格式）
│   └── mbeditor/          # 每个 skill 独立子目录，方便扩展
│       ├── SKILL.md       # 主 Skill 文件（设计原则 + 写作规范）
│       └── api-reference.md  # 完整 API 参考文档
├── skill/                 # 旧路径 → skills/mbeditor/ 的符号链接（向后兼容）
